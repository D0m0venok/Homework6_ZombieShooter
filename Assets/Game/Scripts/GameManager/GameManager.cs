...
namespace ZombieShooter
{
    ...
    public sealed class GameManager : MonoBehaviour
    {
        ...
        public void AddListener(IGameListener listener)
        {
            ...
            if (listener is IGameAddedListener attachElement)
                attachElement.OnAdded();
        ...
        public void RemoveListener(IGameListener listener)
        {
            ...
            if (listener is IGameRemovedListener attachElement)
                attachElement.OnRemoved();
            ...
            switch (state)
            {
                case GameState.PLAYING:
                    ...
                    if (_state == GameState.PAUSED)
                        _listeners.CacheForEach<IGameResumeListener>(listener => listener.OnResume());
                    else
                        _listeners.CacheForEach<IGameStartListener>(listener => listener.OnStart());
                    break;
                case GameState.PAUSED:
                    _listeners.CacheForEach<IGamePauseListener>(listener => listener.OnPause());
                    _listeners.IsEnable = false;
                    break;
                case GameState.FINISHED:
                    _listeners.CacheForEach<IGameFinishListener>(listener => listener.OnFinish());
                    _listeners.IsEnable = false;
                    break;
            }
